...
The three Delulu columns come from a single Delulu checkpoint and reproduce
...
with a different `real_modalities` tuple per path (see delulu.py:274-284).
...
# NOTE: dfc2020_data_utils applies the Copernicus-Bench 8-class mapping (Copernicus-
...
def _delulu_predict(model: EvanSegmenter, batch: dict, modality_bands_dict: dict,
    ...
    """Transfer / peeking / addition predictions from one Delulu checkpoint.

    Mirrors evaluate_multimodal() in delulu.py: a single modality-specific pass is
...
def main() -> None:
    ...
    p.add_argument("--delulu", required=True,
                   help=f"Delulu checkpoint for {STARTING_MODALITY} -> {NEW_MODALITY}.")
