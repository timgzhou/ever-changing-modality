"""
Thin W&B Sweeps wrapper around train_delulu.main().

All training logic lives in train_delulu.py. This file only:
  1. Parses swept + fixed args (using train_delulu's argument names/defaults).
  2. Converts the three boolean-as-string flags that W&B passes as "True"/"False" strings.
  3. Calls train_delulu.main(args).
...
def main():
    parser = argparse.ArgumentParser(description='W&B Sweeps wrapper for Delulu training.')
    ...
    # train_delulu.main() writes these into the results CSV; the sweep wrapper must
