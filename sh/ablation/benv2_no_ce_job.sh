...
#SBATCH --output=logs/shot_ete_ablate_ce/%j.out
...
RESULTS_CSV="res/ablation/benv2_no_ce.csv"
    ...
    awk -F',' -v d="$dataset" -v sm="$start_mod" -v nm="$new_mod" \
        ...
        'function near(a,b) { return (a==0 && b==0) || (a!=0 && (a-b)^2/(a*a) < 1e-6) }
         NR>1 && $1==d && $3==sm && $4==nm && near($5+0,lr+0) && near($6+0,wd+0) \
             ...
             && near($10+0,lf+0) && near($11+0,ls+0) {found=1; exit} END {exit !found}' \
        "$csv"
}
...
echo "=== ${DATASET} | ${MODEL} | ${STARTING_MOD} -> ${NEW_MOD} | ${N_CONFIGS} configs | losses=latent+prefusion+distill lf=0 ==="
...
for IDX in $(seq 0 $((N_CONFIGS - 1))); do
    ...
    # labeled_frequency is fixed at 0 — dedup checks against 0
    LF=0
    ...
    echo "  rank=${RANK} teacher=${TEACHER} labeled_frequency=0"
