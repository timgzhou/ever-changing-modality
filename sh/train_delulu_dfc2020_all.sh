#!/bin/bash
# DeluluNet (train_delulu) on DFC2020, Copernicus-Bench split — all 6 transfer directions.
#
...
#
# Delulu hyperparameters are carried over from the biomassters s1->s2 best config
# and are NOT tuned for DFC2020. Treat this as the first pass, then sweep.
#
# Usage:
#   bash sh/train_delulu_dfc2020_all.sh                 # upernet teachers (default)
#   DECODER=linear bash sh/train_delulu_dfc2020_all.sh  # linear teachers
#   EPOCHS=40 bash sh/train_delulu_dfc2020_all.sh
  ...
  for NEW in s1 s2_rgb s2_norgb; do
    ...
    for LL in ${LAMBDA_LATENTS}; do
      ...
      sbatch --export=ALL,START="${START}",NEW="${NEW}",TEACHER="${TEACHER}",DECODER="${DECODER}",EPOCHS="${EPOCHS}",BATCH_SIZE="${BATCH_SIZE}",LAMBDA_LATENT="${LL}" \
          sh/train_delulu_dfc2020_job.sh
      n=$((n+1))
    done
  done
done
echo "submitted ${n} train_delulu jobs (${DECODER} teachers)"
