...
#SBATCH --output=logs/train_delulu_dfc2020/%j.out
...
# One train_delulu direction on DFC2020 / Copernicus-Bench split.
# Expected env vars (set by sh/train_delulu_dfc2020_all.sh):
...
mkdir -p logs/train_delulu_dfc2020 checkpoints res/delulu

# Delulu hyperparameters from the biomassters s1->s2 best config (untuned here).
...
# see _compute_latent_loss in delulu.py. Every lambda below was tuned against
...
echo "=== train_delulu | dfc2020 (cobench) ${START} -> +${NEW} | decoder=${DECODER} ==="
