...
#SBATCH --output=logs/train_delulu_default/%j.out
...
#
# Deliberately passes NO Delulu hparam flags (modality_dropout, token_mask_ratio,
...
#
# Usage:  sbatch sh/train_delulu_biomassters_default.sh
#         TEACHER=path/to/f0.pt NEW_MOD=s1 EPOCHS=64 sbatch sh/train_delulu_biomassters_default.sh

source sh/env.sh
export TQDM_DISABLE=1
# T=12 folds the time axis into the batch dim (effective batch = batch_size * T),
# so Delulu (student + frozen teacher + decoders + projectors) is memory-heavy.
# Reduce fragmentation and keep batch_size modest.
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
mkdir -p logs/train_delulu_default
...
echo "=== train_delulu DEFAULT params | biomassters -> ${NEW_MOD} | T=${NUM_TIME_STEPS} ==="
