...
#SBATCH --output=logs/train_delulu_best/%j.out
...
#
# Usage:  sbatch sh/train_delulu_biomassters_best.sh              # s1 -> s2 (default)
#         DIRECTION=s2s1 sbatch sh/train_delulu_biomassters_best.sh
#         EPOCHS=64 BATCH_SIZE=16 sbatch sh/train_delulu_biomassters_best.sh
...
mkdir -p logs/train_delulu_best checkpoints
...
echo "=== train_delulu BEST params | biomassters ${DIRECTION} (new_mod=${NEW_MOD}) | T=${NUM_TIME_STEPS} ==="
