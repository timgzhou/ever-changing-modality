...
# Usage: bash sh/train_delulu_sweep.sh
    ...
    for MODEL in "${MODELS[@]}"; do
        ...
        for SELECT_BY in transfer peeking addition; do
            ...
            for _ in $(seq 1 "$N_RUNS"); do
                sbatch sh/train_delulu_sweep_job.sh "$DATASET" "$MODEL" "$STARTING_MOD" "$NEW_MOD" "$TEACHER" "$SELECT_BY"
            done
        done
