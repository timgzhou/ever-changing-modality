...
def main():
    ...
    parser.add_argument('--stage2_fusion_epochs', type=int, default=4,
    ...
    parser.add_argument('--num_supervised_epochs', type=int, default=2,
                        help='Number of epochs for supervised evaluation (default: 8)')
    ...
    wandb.init(
        ...
        name=f"stage2_{config['model_type']}_{newmod}_{args.stage2_train_method}"
    )
    ...
    # ========================================= TRAIN =====================================
    if args.stage2_train_method=="mae":
        print(f"\n Using MAE training method for fusion blocks")
        # bands_target maps modality -> band tuple (for channel counts in decoders)
        bands_target = {
            'rgb': modality_bands_dict['rgb'],
            newmod: modality_bands_dict[newmod],
        }
        # Train fusion with: newmod uses MAE pixel reconstruction, rgb uses latent matching
        train_mae_fusion_phase(
            model=model,
    ...
    checkpoint_stage2 = {
        ...
        'config': {
            **config,
            ...
            'ssl_lr': args.ssl_lr,
            'stage2_fusion_epochs': args.stage2_fusion_epochs,
    ...
    # Freeze everything except classifier (to evaluate fusion quality)
    model.freeze_all()
    model.set_requires_grad('all', classifier=True)
    print(f"Freezing all EVAN components, training classifier only.")

    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.stage2_lr)
    ...
    print(f"{newmod}-only Result: {train_acc_newmod_single=:.2f} {test_acc_newmod_single=:.2f} {best_test_acc_newmod=:.2f} at epoch {best_epoch_newmod}")

    # Log results to CSV
    filename = "train_stage2_res.csv"
    ...
    with open(filename, mode='a', newline='') as file:
        ...
        writer.writerow([
            config['model_type'],
            newmod,
            args.stage2_train_method,
            args.ssl_lr,
            args.stage2_fusion_epochs,
            args.mae_mask_ratio,
