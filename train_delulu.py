...
def _parse_args():
    parser = argparse.ArgumentParser(description='End to end training for Delulu model.')
    # IMPORTANT
    ...
    parser.add_argument('--new_mod_group', type=str, required=True,
                        help='New modality to add (eurosat: vre/nir/swir/rgb; benv2/dfc2020/biomassters: s1/s2)')
...
def main(args=None):
    ...
    # logic may apply to Delulu, whose student is likewise teacher-initialised.
    #
    ...
    # since train_delulu_model() otherwise deep-copies the (already reset) model.
    ...
    # ========================================== TRAIN Delulu ===========================================
    trainable_total, best_checkpoints, best_checkpoint_summary, teacher_baselines = train_delulu_model(
        model=model,
    ...
    if args.save_checkpoint:
        ...
        if args.checkpoint_name:
            ...
            timestamp_delulu = datetime.now().strftime('%m%d_%H%M')
...
python -u train_delulu.py \
    ...
    --batch_size 32 \
    --results_csv res/delulu_benv2.csv \
...
python -u train_delulu.py \
    ...
    --eval_every_n_epochs 2 \
    --batch_size 32 \
    --results_csv res/delulu_dfc2020.csv \
    --active_losses latent prefusion distill ce \
